...
async function placeBet(pm: any, addr: string, signer: any, id: bigint, amount: number, side: boolean) {
  const enc = await fhevm.createEncryptedInput(addr, signer.address).add64(amount).addBool(side).encrypt();
...
async function main() {
  ...
  const markets: Array<{ q: string; bets: Array<[number, boolean]> }> = [
    {
      q: "Will BTC close above $100k on July 1, 2026?",
      bets: [
        [100, true],
        [40, false],
      ],
    ...
    {
      q: "Will the Zama Builder Track get over 50 submissions?",
      bets: [
        [75, true],
        [60, false],
      ],

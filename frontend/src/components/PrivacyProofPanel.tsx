...
const POINTS: { icon: string; text: string }[] = [
  ...
  {
    icon: '⚖️',
    text: 'Both pool totals update on every bet regardless of side — no side-channel leak.',
  },
  ...
  return (
    <div className="card overflow-hidden">
      ...
      >
        ...
        <span
          className={`text-mute text-xs transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
        >
          ▾
        </span>
      </button>
              ...
              >
                {POINTS.map(p => (
                  <motion.li
                    key={p.text}
                    variants={fadeUp}
                    className="flex gap-3 text-[13px] text-body leading-snug"
                  >

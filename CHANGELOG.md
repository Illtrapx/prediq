...
- Showcase demo seeding (`VITE_DEMO_TRADES`, default `on`) — synthetic recent bets in the activity feed of live markets
  and synthetic wallets in the leaderboard. Purely client-side display; no chain writes or persistence. Set
  `VITE_DEMO_TRADES=off` to show real on-chain data only.
